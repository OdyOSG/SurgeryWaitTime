...
executeTimeToEvent <- function(con,
  ...
  for (i in seq_along(thFiles)) {
    ...
    # Job log
    cli::cat_line()
    cli::cat_bullet(crayon::magenta("Execute time to discontinuation for cohort id: "), targetCohortId[i], " (Cohort name: ",  targetCohortName[i], ")",
